...
async def clear_webhook():
    """Clear any existing Telegram webhook before starting polling."""
    bot = Bot(token=TOKEN)
    await bot.delete_webhook(drop_pending_updates=True)
    logging.info("🔌 Webhook cleared before polling started.")

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Respond to the /start command with a welcome message and schedule tips."""
...
async def scheduled_tips_job(context: ContextTypes.DEFAULT_TYPE):
    """Send daily tips to the chat when the scheduled job runs."""
